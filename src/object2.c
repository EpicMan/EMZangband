...
static void a_m_aux_3(object_type *o_ptr, int level, byte flags)
{
	...
	if (o_ptr->tval == TV_RING)
	{
		...
		case SV_RING_SPEED:
			o_ptr->pval = randint1(o_ptr->pval / 2) + m_bonus(o_ptr->pval, level);
		...
		case SV_RING_LORDLY:
			...
			while (one_in_(4)) add_ego_power(EGO_XTRA_HI_RESIST, o_ptr);
			o_ptr->to_a = rand_range(10, 15) + m_bonus(10, level);
			rating_boost = 5;
			break;

		case SV_RING_ATTACKS:
			if (one_in_(7)) o_ptr->pval = 2;
			allow_curse = TRUE;
			break;

		case SV_RING_CAT:
			o_ptr->pval = 1 + m_bonus(o_ptr->pval, level);
			if (one_in_(3)) SET_FLAG(o_ptr, TR_SUST_DEX);
			allow_curse = TRUE;
			break;
		}
	}
	else if (o_ptr->tval == TV_AMULET)
	{
		...
		case SV_AMULET_THE_MAGI:
			...
			if (one_in_(3)) SET_FLAG(o_ptr, TR_SLOW_DIGEST);
			rating_boost = 25;
			break;
		...
		case SV_AMULET_RESISTANCE:
			if (one_in_(3)) add_ego_power(EGO_XTRA_HI_RESIST, o_ptr);
			if (one_in_(5)) SET_FLAG(o_ptr, TR_RES_POIS);
			break;
	...
	else if (o_ptr->tval == TV_SOFT_ARMOR && o_ptr->sval == SV_HAGAROMO)
	{
		o_ptr->to_a = o_ptr->to_a + randint1(10) + m_bonus(10, level);
	}
