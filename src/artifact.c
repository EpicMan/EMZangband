...
void create_named_art(int a_idx, int x, int y)
{
	...
	case ART_NIMTHANC:
	case ART_DETHANC:
		add_ego_power(EGO_XTRA_HI_RESIST, q_ptr);
		break;

	/* Two random high resists */
	case ART_ARVEDUI:
		add_ego_power(EGO_XTRA_HI_RESIST, q_ptr);
		add_ego_power(EGO_XTRA_HI_RESIST, q_ptr);
		break;
	...
	/* Random high resist + random ability*/
	case ART_POWER:
	case ART_GONDOR:
	case ART_AULE:
	case ART_MISER:
	...
	case ART_TERROR:
		...
		{
			SET_FLAG(q_ptr, TR_CURSED);
			SET_FLAG(q_ptr, TR_HEAVY_CURSE);
			SET_FLAG(q_ptr, TR_AGGRAVATE);
			SET_FLAG(q_ptr, TR_TY_CURSE);
		}
		break;

	case ART_STING:
		add_ego_power(EGO_XTRA_HI_RESIST, q_ptr);
		if (p_ptr->rp.prace == RACE_HOBBIT)
		{
			q_ptr->pval = 4;
			q_ptr->to_h = 14;
		}
	...
	case ART_MERLIN:
		/* Dependent power : +2 sp for mages and high mages*/
		/* Unique power : +1 dam per 10 maximum sp(applied after other bonuses).*/
		if (p_ptr->rp.pclass == CLASS_MAGE || p_ptr->rp.pclass == CLASS_HIGH_MAGE)
			SET_FLAG(q_ptr, TR_SP);
		break;
...
void activate_artifact(object_type* o_ptr)
{
	int dir = 0;
	...
	case ART_CARLAMMAS:
		...
		inc_protevil(randint1(25) + 3 * p_ptr->lev);
	...
	case ART_RAZORBACK:
		...
		scatter_ball(damroll(5, 3), GF_ELEC, 1000, 3);
		o_ptr->timeout = 100;
		break;
	...
	case ART_BLADETURNER:
		if (get_aim_dir(&dir))
		{
			...
			clear_afraid();
			inc_shero(rand_range(50, 100));
			hp_player(30);
			inc_blessed(rand_range(50, 100));
			inc_oppose_acid(rand_range(50, 100));
			inc_oppose_elec(rand_range(50, 100));
			inc_oppose_fire(rand_range(50, 100));
			inc_oppose_cold(rand_range(50, 100));
			inc_oppose_pois(rand_range(50, 100));
			o_ptr->timeout = 100;
		}
		break;
	...
	case ART_ARVEDUI:
		msgf("Your armor shines with a brilliant white light.");
	...
	case ART_HITHLOMIR:
		msgf("Your robe shines brightly.");
		cure_wounds(500, TRUE, TRUE, TRUE);
		dir = randint1(100);
		if (dir <= 50)
			inc_blessed(rand_range(25, 50));
		else if (dir <= 75)
			turn_monsters(p_ptr->lev * 4);
		else
			summon_specific(-1, p_ptr->px, p_ptr->py, p_ptr->lev, SUMMON_ANGEL, TRUE, TRUE, TRUE);
		o_ptr->timeout = 1000;
		break;
	...
	case ART_EONWE:
		msgf("Your great axe lets out a long, shrill note...");
		mass_genocide(TRUE);
		o_ptr->timeout = 1000;
		break;
	...
	case ART_DAWN:
		...
		summon_specific(-1, p_ptr->px, p_ptr->py, p_ptr->depth, SUMMON_DAWN, TRUE, TRUE, TRUE);
	...
	case ART_WHIRLWIND:msgf("Your ball and chain swings through the air...");
		whirlwind_attack();
		o_ptr->timeout = rand_range(50, 100);
		break;
	...
	case ART_JADE_DRAGON:
		msgf("Your cloak glows bright green...");
		inc_fast(rand_range(75, 150));
		o_ptr->timeout = rand_range(150, 300);
		break;
	}
