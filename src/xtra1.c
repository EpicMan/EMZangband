...
/*Apply special object bonuses that used to be done in LUA (TRIGGER_BONUS in code, L:BONUS: in edit files */
static void object_special_bonuses(const object_type* o_ptr, bonuses_type* b)
{
	if (o_ptr->a_idx)
	{
		switch (o_ptr->a_idx)
		{
			/* Terror Mask: +1 STR per 5 to-AC (+2) */
		case ART_TERROR:
			b->stat[A_STR] += o_ptr->to_a / 5;
			break;

			/*Thief's Shadow: +1 Stealth per 2 to-AC (+2) */
		case ART_HOLCOLLETH:
			b->skills[SKILL_STL] += o_ptr->to_a / 2;
			break;

			/* Thorongil: +1 searching/perception per 10 levels */
		case ART_THORONGIL:
			b->skills[SKILL_SNS] += p_ptr->lev / 10;
			b->skills[SKILL_FOS] += p_ptr->lev / 10;
			break;

			/*Tuor, rogues get DEX+3 +lev/10 */
		case ART_TUOR:
			if (p_ptr->rp.pclass == CLASS_ROGUE)
				b->stat[A_DEX] += 3 + p_ptr->lev / 10;
			break;

			/*Bleys: +1 speed plus 1 per 5 levels */
		case ART_BLEYS:
			b->pspeed = (p_ptr->lev + 4) / 5;
			break;

			/*Narthanc: +1 STR plus 1 per 5 levels */
		case ART_NARTHANC:
			b->stat[A_STR] = (p_ptr->lev + 4) / 5;
			break;

			/*Nimthanc: +1 CON plus 1 per 5 levels */
		case ART_NIMTHANC:
			b->stat[A_CON] = (p_ptr->lev + 4) / 5;
			break;

			/*Dethanc: +1 DEX plus 1 per 5 levels */
		case ART_DETHANC:
			b->stat[A_DEX] = (p_ptr->lev + 4) / 5;
			break;

			/*Rilia: +1 INT plus 1 per 10 levels */
		case ART_RILIA:
			b->stat[A_INT] = (p_ptr->lev + 9) / 10;
			break;

			/* Hagen: +1 speed per 5 levels (round up) */
		case ART_NIMLOTH:
			b->pspeed = (p_ptr->lev + 4) / 5;
			break;

			/* Luck Blade: +10 saves, +1 per +2 to-hit or to-dam */
		case ART_LUCK_BLADE:
			b->skills[SKILL_SAV] += (o_ptr->to_h + o_ptr->to_d) / 2;
			break;

			/* Jade Dragon: Monks get +1 speed per 10 levels */
		case ART_JADE_DRAGON:
			if (p_ptr->rp.pclass == CLASS_MONK)
				b->pspeed = (p_ptr->lev + 9) / 10;
			break;
		}
	}
}
...
void object_bonuses(const object_type *o_ptr, bonuses_type *b)
{
	...
	/* Apply special bonuses (TRIGGER_BONUS/L:BONUS in LUA) */
	object_special_bonuses(o_ptr, b);
}
...
static void calc_bonuses(void)
{
	...
	for (i = 0; i < EQUIP_MAX; i++)
	{
		...
		if (o_ptr->tval == TV_AMULET && o_ptr->sval == SV_AMULET_SUSTENANCE)
			SET_FLAG(p_ptr, TR_SUSTENANCE);
	}
